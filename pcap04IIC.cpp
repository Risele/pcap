...
bool PCAP04IIC::test_connection()
{
  ...
  Serial.print("Opcode raw value: 0x");
  Serial.println(test_read.opcode,HEX);

  Serial.print("Opcode modified value: 0x");
  Serial.println(((test_read.opcode >> 8) & 0xFF),HEX);
...
//  SPI.beginTransaction(pcap_spi_settings);
  ...
  if ((addr >= 0) && (addr < PCAP_RESULTS_SIZE + PCAP_STATUS_SIZE))
  {
   ...
   // Serial.print("Sending opcode 0x");
   // Serial.println(rd_result.opcode);
